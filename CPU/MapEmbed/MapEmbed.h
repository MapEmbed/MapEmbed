...
private:
    ...
    BOBHash32 hash_to_cell[MAX_LAYER];
    BOBHash32 hash_to_bucket[M];
    BOBHash32 hash_to_guide;
...
private:
    ...
    int calculate_guide(const char* key, int cell_value){
        // cost: calculate 1 hash
        uint tmp = hash_to_guide.run(key, KEY_LEN*sizeof(char));
